...
var MIME_OEMBED_TEXT_JSON = exports.MIME_OEMBED_JSON = 'text/json+oembed';
...
exports.fetch = function(url, parameters, cb) {
	exports.discover(url, function(error, alternates) {
		var oembedUrl;
		if (alternates && (alternates[MIME_OEMBED_JSON] || alternates[MIME_OEMBED_TEXT_JSON])) {
			var urlToUse;
			if (alternates[MIME_OEMBED_JSON]){

				urlToUse = alternates[MIME_OEMBED_JSON];
			}else if (alternates[MIME_OEMBED_TEXT_JSON]){
				urlToUse = alternates[MIME_OEMBED_TEXT_JSON];
			}else{
				throw new Error('Missing JSON mime type');
			}
			oembedUrl = applyParameters(urlToUse, parameters);
